...
public class ClientHandler extends Thread {
	...
	public ClientHandler(MetroSystem bnk, Socket s) {
		this.bnk = bnk;
		this.s = s;
		this.work = true;
		try {
	...
	private void processOperation(Object obj) throws IOException, ClassNotFoundException {
		if (obj instanceof StopOperation) {
			...
			showBalance(obj);
		} else if (obj instanceof GetUserOperation) {
			getUser(obj);
		}
	...
	private void addCard(Object obj) throws IOException, ClassNotFoundException {
		Card card = ((AddMetroCardOperation) obj).getCrd();
		bnk.addCard(card);
		os.writeObject("Card Added: " + card.getSerialNumber());
		os.flush();
	}
	...
	private void payMoney(Object obj) throws IOException, ClassNotFoundException {
		...
		boolean res = bnk.payByCard(op.getSerNum(), op.getMoney());
		if (res) {
	...
	private void showBalance(Object obj) throws IOException, ClassNotFoundException {
		...
		Optional<Card> card = bnk.findCard(op.getSerNum());
		if (card.isPresent()) {
			os.writeObject(card.get().getBalance());
			os.flush();
		} else {
	...
	private void getUser(Object obj) throws IOException, ClassNotFoundException {
		GetUserOperation op = (GetUserOperation) obj;
		Optional<Card> card = bnk.findCard(op.getSerNum());
		if (card.isPresent()) {
			os.writeObject(card.get().getUser());
			os.flush();
		} else {
			os.writeObject("Cannot find user for Card: " + op.getSerNum());
		}
	}
